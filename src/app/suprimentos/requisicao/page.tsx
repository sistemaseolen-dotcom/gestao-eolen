...
export default async function RequisicaoPage({
  ...
  const { data: requisicoes, count, error } = await query;
  const totalPages = count ? Math.max(1, Math.ceil(count / PAGE_SIZE)) : 1;
  const pages = pageWindow(page, totalPages);
  const linkFor = (p: number) => `/suprimentos/requisicao?${new URLSearchParams({ ...(q ? { q } : {}), page: String(p) })}`;

  return (
    <main className="p-6">
      <h1 className="mb-4 text-xl font-semibold">Requisição de Material</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-t-lg border border-neutral-200 bg-white px-3 py-2">
        <Link href="/suprimentos/requisicao/novo" className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 text-sm text-neutral-700 hover:bg-neutral-50">
          + Inserir
        </Link>
        <form action="/suprimentos/requisicao" className="flex items-center gap-2 text-sm text-neutral-600">
          ...
          {q && <Link href="/suprimentos/requisicao" className="text-neutral-500 hover:underline">limpar</Link>}
        </form>
      </div>
          ...
          <tbody>
            {(requisicoes || []).map((r: any) => (
              <tr key={r.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                <td className="px-4 py-3 font-medium text-[#a7332a]">{r.tipos_produto?.nome || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{r.codigo || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{r.sku || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{r.estoque_fisico ?? "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{r.unidade || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{r.localizacao || "-"}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3 text-neutral-400">
                    <Link href={`/suprimentos/requisicao/${r.id}/editar`} title="Editar" className="hover:text-[#a7332a]">
                      ✏️
                    </Link>
                    <DeleteButton
                      action={async () => {
                        "use server";
                        await deleteRequisicao(r.id);
                      }}
                      confirmMessage="Excluir esta requisição?"
                    />
                  </div>
                </td>
              </tr>
            ))}
            {!error && (requisicoes || []).length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-neutral-500">Nenhuma requisição encontrada.</td>
              </tr>
            )}
          </tbody>
        </table>
