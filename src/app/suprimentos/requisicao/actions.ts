...
const BASE_PATH = "/suprimentos/requisicao";
...
export async function createRequisicao(formData: FormData) {
  ...
  const { error } = await supabase.from("requisicoes_material").insert(buildPayload(formData));
  if (error) throw new Error(error.message);
  revalidatePath(BASE_PATH);
  redirect(BASE_PATH);
}

export async function updateRequisicao(id: string, formData: FormData) {
  const supabase = createServiceClient();
  const { error } = await supabase.from("requisicoes_material").update(buildPayload(formData)).eq("id", id);
  if (error) throw new Error(error.message);
  revalidatePath(BASE_PATH);
  redirect(BASE_PATH);
}
