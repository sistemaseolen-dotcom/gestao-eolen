...
export default async function VeiculosPage({
  ...
  let query = supabase
    .from("veiculos")
    .select(
      `id, placa, contrato, status, regional, km_atual,
       locadoras ( nome ),
       pessoas ( nome ),
       projetos ( nome )`,
      { count: "exact" }
  ...
  const { data: veiculos, count, error } = await query;
  const totalPages = count ? Math.max(1, Math.ceil(count / PAGE_SIZE)) : 1;
  const pages = pageWindow(page, totalPages);
  const linkFor = (p: number) => `/gestao-frotas/veiculos?${new URLSearchParams({ ...(q ? { q } : {}), page: String(p) })}`;

  return (
    <main className="p-6">
      <h1 className="mb-4 text-xl font-semibold">Veículos</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-t-lg border border-neutral-200 bg-white px-3 py-2">
        <Link href="/gestao-frotas/veiculos/novo" className="rounded-md border border-neutral-300 bg-white px-3 py-1.5 text-sm text-neutral-700 hover:bg-neutral-50">
          + Inserir
        </Link>
        <form action="/gestao-frotas/veiculos" className="flex items-center gap-2 text-sm text-neutral-600">
          ...
          {q && <Link href="/gestao-frotas/veiculos" className="text-neutral-500 hover:underline">limpar</Link>}
        </form>
      </div>
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-neutral-200 bg-neutral-50">
            <tr>
              ...
              <th className="px-4 py-3 font-medium text-neutral-500">Contrato</th>
              <th className="px-4 py-3 font-medium text-neutral-500">Locadora</th>
              <th className="px-4 py-3 font-medium text-neutral-500">Condutor</th>
              ...
              <th className="px-4 py-3 font-medium text-neutral-500">Km atual</th>
          ...
          <tbody>
            {(veiculos || []).map((v: any) => (
              <tr key={v.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                <td className="px-4 py-3 font-medium text-[#a7332a]">{v.placa || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.contrato || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.locadoras?.nome || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.pessoas?.nome || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.projetos?.nome || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.regional || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.status || "-"}</td>
                <td className="px-4 py-3 text-neutral-600">{v.km_atual ?? "-"}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3 text-neutral-400">
                    <Link href={`/gestao-frotas/veiculos/${v.id}/editar`} title="Editar" className="hover:text-[#a7332a]">
                      ✏️
                    </Link>
                    <DeleteButton
                      action={async () => {
                      ...
                      confirmMessage={`Excluir veículo ${v.placa}?`}
                    />
                  </div>
                </td>
              </tr>
            ))}
            {!error && (veiculos || []).length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-10 text-center text-neutral-500">Nenhum veículo encontrado.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="mt-3 flex items-center justify-between text-sm text-neutral-600">
        <span>{count ?? 0} veículo(s) · Página {page} de {totalPages}</span>
